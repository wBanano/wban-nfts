...
const createClaim = async (row, index) => {
	...
	row.uuid = Date.now();
	row.expiry = Math.round(row.uuid / 1_000) + Number.parseInt(process.env.CLAIM_EXPIRY);
...
exports.handler = async (event) => {
	...
	try {
		...
		for (let i = 0; i < rows.length; i++) {
			const item = createClaim(rows[i]);
			console.log(`[${i + 1}/${rows.length}] ${JSON.stringify(rows[i])}`);
			batch.push(item);
